...
width_ground = 150.0
buried_depth = 16.0
spans = np.array([6, 6, 6, 6])
floors = np.array([5, 5, 6, 6])

# mesh parameters [0.2, 1.0]
mesh_seed_structure = 0.75
...
mesh_coef = 0.0053 * mesh_seed_structure ** 3 + 0.012 * mesh_seed_structure ** 2 + 0.0067 * mesh_seed_structure + 1.006
...
# 左右侧水平线
n0 = int(np.ceil(width_ground / 3 / mesh_seed_structure))
for i in range(floor_num):
...
# 上侧竖向线
n1 = int(np.ceil(buried_depth / mesh_seed_structure)) + 1
for i in range(span_num):
...
# 下侧竖向线
n2 = int(np.ceil((height_ground - buried_depth - h) / 1.5 / mesh_seed_structure)) + 1
for i in range(span_num):
...
# 水平结构线 (四组)
for i in range(span_num):
    n3 = int(np.ceil(spans[i] / mesh_seed_structure)) + 1
...
# 竖直结构线 (四组)
for i in range(floor_num):
    n4 = int(np.ceil(floors[i] / mesh_seed_structure)) + 1
    print(n4)
