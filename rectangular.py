...
class MeshRectangular:
    ...
    @staticmethod
    def mesh(height, width, buried, span, floor, mesh_seed_soil=1.2, mesh_seed_structure=1.):
        """
        :param mesh_seed_structure:
        :param mesh_seed_soil:
        :param height: 90.0
        :param width: 150.0
        :param buried: 20.0
        :param span: np.array([30, 5, 10, 5])
        :param floor: np.array([10, 5, 10])
        :return:
        """
        gmsh.initialize()
        ...
        floors = floor
        # 土体网格密度（推荐0.2-1.5）
        mesh_seed_soil = mesh_seed_soil
        # 结构网格密度
        mesh_seed_structure = mesh_seed_structure
        ...
        gmsh.write('mesh_rectangular.jpg')
        gmsh.fltk.run()
        gmsh.write('mesh_rectangular.msh')
        # 结束gmsh
        gmsh.finalize()
...
if __name__ == '__main__':
    mr = MeshRectangular()
    mr.mesh(height=70.0, width=140.0, buried=12.0, span=[3, 3, 5], floor=[5, 6])
